...
app.post('/booking', async (req, res) => {
  ...
  try {
    ...
    console.log('Received data:', { name, email, contact, date, message, user_id });

    const query = 'INSERT INTO booking (name, email, contact, date, message, user_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id';
    const params = [name, email, contact, date, message, user_id];
    const result = await client.query(query, params);

    const data = { id: result.rows[0].id, name, email, contact, date, message, user_id };

    console.log(`Booking created successfully with id ${data.id}`);
    res.json({ status: "success", data, message: "Booking created successfully" });
  } catch (error) {
...
//Read the booking info with GET
...
app.put('/booking/:id', async (req, res) => {
  ...
  try {
    ...
    const updateQuery = 'UPDATE booking SET name = $1, email = $2, contact = $3, date = $4, message = $5 WHERE id = $6 RETURNING *';
    const queryData = [updatedData.name, updatedData.email, updatedData.contact, updatedData.date, updatedData.message, id];
